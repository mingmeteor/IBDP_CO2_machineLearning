...
for month in range(1,50):  
    ...
    for i, testdata in enumerate(X_test_roll, 0):
        ...
        x_permeability_recovered = X_test_case[0,:,:,:]*(perm_xyz_max-perm_xyz_min) + perm_xyz_min
        
        y_permeability_recovered = X_test_case[1,:,:,:]*(perm_xyz_max-perm_xyz_min) + perm_xyz_min
        
        z_permeability_recovered = X_test_case[2,:,:,:]*(perm_xyz_max-perm_xyz_min) + perm_xyz_min
        ...
        if i < plot_cases:
            ...
            h1=axs[0,0].pcolormesh(x_coordinates[:,plot_layer,:], z_coordinates[:,plot_layer,:],  x_permeability_recovered[:,plot_layer,:],\
                                   cmap='rainbow')                
            axs[0,0].set_title('X permeability (mD)',fontsize = 20)
            #axs[0,0].axis('off')
...
for i in range(2,test_cases):
    ...
    axs[0,0].plot(np.arange(2,51), saturation_test_recovered_max[i,1:],\
      linewidth = 5, color = colors[i-2], label = f'#{testCaseId}')
...
axs[0,0].plot(np.arange(2,51), saturation_test_recovered_max[1,1:], \
  '--', linewidth = 5, color = '#bcbd22', label = '#9')    
axs[0,0].plot(np.arange(2,51), saturation_test_recovered_max[0,1:], \
  ':', linewidth = 5, color = '#17becf', label = '#10') 
...
axs[1].yaxis.set_ticks([-0.18, -0.14, -0.10, -0.06, -0.02])
axs[1].set_yticklabels([-0.18, -0.14, -0.10, -0.06, -0.02])
axs[1].set_xlabel('Month',fontsize = 24)
axs[1].set_ylabel('Prediction minimum',fontsize = 24)      
axs[1].legend(loc = 'lower left', ncol=4,\
    handletextpad=0.2,labelspacing=0.2,columnspacing=0.2,
    fontsize = 20, frameon=False)  
