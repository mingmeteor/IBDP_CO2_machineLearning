...
for epoch in range(epochs):
    ...
    for i, traindata in enumerate(X_train_batch, 0):
        ...
        if penalty_1 < 50e10:
            penalty_1 = 1.2*penalty_1
        if penalty_2 < 50e10:
            penalty_2 = 1.2*penalty_2
        
        if penalty_3 < 1e10:
            penalty_3 = 1.2*penalty_3           
        ...
        if i==len(X_train_batch)-1:
            ...
            logline = "Epoch: {}, batch-idx: {}, penalty_1: {}\n".format(epoch, i, penalty_1)
            print(logline)
            
            logline = "Epoch: {}, batch-idx: {}, compareLoss: {}\n".format(epoch, i, compareLoss)
            print(logline)
            
            logline = "Epoch: {}, batch-idx: {}, L1: {}\n".format(epoch, i, lossL1Train)
            print(logline)
            
            logline = "Epoch: {}, batch-idx: {}, L1_orig: {}\n".format(epoch, i, lossL1_orig.item())
            print(logline)
            
            logline = "Epoch: {}, batch-idx: {}, L1_penalty: {}\n".format(epoch, i, lossL1_penalty.item())
            print(logline)
            
            logline = "Epoch: {}, batch-idx: {}, L1_penalty_noFactor: {}\n".format(epoch, i, lossL1_penalty_noFactor.item())
            print(logline)
            ...
            f_trainError.flush()
            f_trainError.write('penalty factor 1: '+str(penalty_1)+'\n')
            f_trainError.flush()
            f_trainError.write('compareLoss: '+str(compareLoss)+'\n')
            f_trainError.flush()
            f_trainError.write('total: '+str(lossL1Train)+'\n')
            f_trainError.flush()
            f_trainError.write('L1_orig: '+str(lossL1_orig.item())+'\n')
            f_trainError.flush()
            f_trainError.write('penalty: '+str(lossL1_penalty.item())+'\n')
            f_trainError.flush()
            f_trainError.write('penalty_noFactor: '+str(lossL1_penalty_noFactor.item())+'\n')
            f_trainError.flush()
            
            f_trainErrorOnly.write(theTime+'\n')
            f_trainErrorOnly.flush()
            f_trainErrorOnly.write('L1_orig: '+str(lossL1_orig.item())+'\n')
            f_trainErrorOnly.flush()
    ...
    for i, validata in enumerate(X_validate_batch, 0):
        ...
        lossL1_penalty_validate = torch.abs(torch.minimum(gen_out, zero_array_temp)) \
            + torch.maximum(gen_out-drainageSatMaxdata, zero_array_temp)                        
        lossL1_penalty_validate = lossL1_penalty_validate.sum()/(batch_size_validate*y_validate.shape[1]*y_validate.shape[2]*y_validate.shape[3])
        ...
        if i==len(X_validate_batch)-1:
            ...
            logline = "Epoch: {}, batch-idx: {}, L1_penalty_validate: {}\n".format(epoch, i, lossL1_penalty_validate.item())
            print(logline)
            ...
            f_validateError.flush()
            f_validateError.write('Epoch:' +str(epoch)+', batch-id:' +str(i)+', validation penalty: '+str(lossL1_penalty_validate.item())+'\n')
            f_validateError.flush()
    ...
    f_trainError_batchAverage.flush()           
    f_trainError_batchAverage.write('Epoch:' +str(epoch)+', average train penalty: '+str(L1_penalty_accum)+'\n') 
    f_trainError_batchAverage.flush()
    ...
    f_validateError_batchAverage.flush()                     
    f_validateError_batchAverage.write('Epoch:' +str(epoch)+', average validation penalty: '+str(L1val_penalty_accum)+'\n') 
    f_validateError_batchAverage.flush()           
