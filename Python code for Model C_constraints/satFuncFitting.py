...
import numpy as np
def satFuncFitting(saturation_dataset,drainage_satTable):
    ...
    sat_time = saturation_dataset.shape[4]
       
    def calculateCapi_rePerm(t):
        capi_drainage_array = np.zeros([sat_num,sat_xSize,sat_ySize,sat_zSize])
        rePerm_drainage_array = np.zeros([sat_num,sat_xSize,sat_ySize,sat_zSize])
        for n in range(sat_num):
            for i in range(sat_xSize):
                for j in range(sat_ySize):
                    for k in range(sat_zSize):
                        satValue = saturation_dataset[n,i,j,k,t]                        
################################################################################################################
                        if drainage_satTable[n,i,j,k] == 1:
                            ...
                            if satValue >= 0 and satValue <= 0.6385:
                                ...
                                capi_drainage_array[n,i,j,k] = (capi_drainage - capi_min)/(capi_max - capi_min)
    ...
    ################################################################################################################
                        ...
                        if drainage_satTable[n,i,j,k] == 1:
                            if satValue > 0:
                                ...
                                rePerm_drainage_array[n,i,j,k] = (rePerm_drainage - rePerm_min)/(rePerm_max - rePerm_min)
                        elif drainage_satTable[n,i,j,k] == 2:
                            if satValue > 0:
    ...
    results = Parallel(n_jobs=-1)(delayed(calculateCapi_rePerm)(t) for t in range(sat_time))      
